...
v_cpu_host_week_avg number := 0;
v_cpu_host_week_max number := 0;
v_IOPS_week_avg number := 0;
v_IOPS_week_max number := 0;
...
v_cpu_host_daily_avg arrayOfNumber := arrayOfNumber(0,0,0,0,0,0,0);
v_cpu_db_daily_max arrayOfNumber := arrayOfNumber(0,0,0,0,0,0,0);
v_cpu_host_daily_max arrayOfNumber := arrayOfNumber(0,0,0,0,0,0,0);
v_IOPS_daily_avg arrayOfNumber := arrayOfNumber(0,0,0,0,0,0,0);
v_IOPS_daily_max arrayOfNumber := arrayOfNumber(0,0,0,0,0,0,0);
	...
	if (&V_diagnostic_pack_utilizations>0) then
		...
		loop			
			...
			if trunc(sysdate)-trunc(metric_row.end_time) < &&WEEK then		
				...
				v_cpu_host_week_avg := v_cpu_host_week_avg + nvl(round(metric_row.h_cpu_per_s,2),0);		
				v_IOPS_week_avg := v_IOPS_week_avg + nvl(round((metric_row.read_iops+metric_row.write_iops),2),0);		
				...
				if(metric_row.h_cpu_per_s_max>=v_cpu_host_week_max) then 
					v_cpu_host_week_max := round(metric_row.h_cpu_per_s_max,2); 
				end if;			
				if((metric_row.read_iops_max+metric_row.write_iops_max)>=v_IOPS_week_max) then 
					v_IOPS_week_max := round((metric_row.read_iops_max+metric_row.write_iops_max),2);
				end if;			
				...
				if day_number<7 then 
					...
					v_cpu_host_daily_avg(day_number+1) := v_cpu_host_daily_avg(day_number+1) + nvl(round(metric_row.h_cpu_per_s,2),0);
					v_IOPS_daily_avg(day_number+1) := v_IOPS_daily_avg(day_number+1) + nvl(round(metric_row.read_iops+metric_row.write_iops,2),0);
					...
					if(metric_row.h_cpu_per_s_max>=v_cpu_host_daily_max(day_number+1)) then
						v_cpu_host_daily_max(day_number+1) := round(metric_row.h_cpu_per_s_max,2); 
					end if;
					if(metric_row.read_iops_max+metric_row.write_iops_max>=v_IOPS_daily_max(day_number+1)) then
						v_IOPS_daily_max(day_number+1) := round(metric_row.read_iops_max+metric_row.write_iops_max,2);
					end if;
		...
		if(day_week_count>=&&WEEK_SOFT_LIMIT) then
			...
			if(snap_count_week>0) then
				...
				v_cpu_host_week_avg := round((v_cpu_host_week_avg / snap_count_week),2);
				v_IOPS_week_avg := round((v_IOPS_week_avg / snap_count_week),2);
				v_IOMB_week_avg := round((v_IOMB_week_avg / snap_count_week),2);
			end if;
			--v_cpu_db_week_avg,v_cpu_db_week_max,v_cpu_host_week_avg,v_cpu_host_week_max,v_IOPS_week_avg,v_IOPS_week_max,v_IOMB_week_avg,v_IOMB_week_max
			DBMS_OUTPUT.PUT_LINE(v_cpu_db_week_avg || '|||' || v_cpu_db_week_max || '|||' || v_cpu_host_week_avg || '|||' || v_cpu_host_week_max || '|||' || v_IOPS_week_avg || '|||' || v_IOPS_week_max || '|||' || v_IOMB_week_avg || '|||' || v_IOMB_week_max);
		else
			DBMS_OUTPUT.PUT_LINE('N/A|||N/A|||N/A|||N/A|||N/A|||N/A|||N/A|||N/A');
		end if;
		...
		--the array starts from position 1 (today->position 1 in the array, 7 days ago->position 7 in the array)
			if(daily_count(l_index)>0) then
				...
				v_cpu_host_daily_avg(l_index) := round(v_cpu_host_daily_avg(l_index) / daily_count(l_index),2);
				...
				--v_cpu_db_daily_avg(l_index),v_cpu_db_daily_max(l_index),v_cpu_host_daily_avg(l_index),v_cpu_host_daily_max(l_index),v_IOPS_daily_avg(l_index),v_IOPS_daily_max(l_index),v_IOMB_daily_avg(l_index),v_IOMB_daily_max(l_index)
				DBMS_OUTPUT.PUT_LINE(v_cpu_db_daily_avg(l_index) || '|||' || v_cpu_db_daily_max(l_index) || '|||' || v_cpu_host_daily_avg(l_index) || '|||' || v_cpu_host_daily_max(l_index) || '|||' || v_IOPS_daily_avg(l_index) || '|||' || v_IOPS_daily_max(l_index) || '|||' || v_IOMB_daily_avg(l_index) || '|||' || v_IOMB_daily_max(l_index));
			else
				DBMS_OUTPUT.PUT_LINE('N/A|||N/A|||N/A|||N/A|||N/A|||N/A|||N/A|||N/A');
			end if;		
		END LOOP;
