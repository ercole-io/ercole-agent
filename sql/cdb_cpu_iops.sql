...
v_cpu_host_month_avg number := 0;
v_cpu_host_month_max number := 0;
v_IOPS_month_avg number := 0;
v_IOPS_month_max number := 0;
...
v_cpu_host_weekly_avg arrayOfNumber_weeks := arrayOfNumber_weeks(0,0,0,0);
v_cpu_db_weekly_max arrayOfNumber_weeks := arrayOfNumber_weeks(0,0,0,0);
v_cpu_host_weekly_max arrayOfNumber_weeks := arrayOfNumber_weeks(0,0,0,0);
v_IOPS_weekly_avg arrayOfNumber_weeks := arrayOfNumber_weeks(0,0,0,0);
v_IOPS_weekly_max arrayOfNumber_weeks := arrayOfNumber_weeks(0,0,0,0);
...
v_cpu_host_daily_avg arrayOfNumber_days := arrayOfNumber_days(0,0,0,0,0,0,0);
v_cpu_db_daily_max arrayOfNumber_days := arrayOfNumber_days(0,0,0,0,0,0,0);
v_cpu_host_daily_max arrayOfNumber_days := arrayOfNumber_days(0,0,0,0,0,0,0);
v_IOPS_daily_avg arrayOfNumber_days := arrayOfNumber_days(0,0,0,0,0,0,0);
v_IOPS_daily_max arrayOfNumber_days := arrayOfNumber_days(0,0,0,0,0,0,0);
	...
	if (&V_diagnostic_pack_utilizations>0 AND &V_SNAP_DAYS_RETRIEVED>0) then
		...
		loop		
			...
			if  current_day_difference < &&MONTH then
				...
				v_cpu_host_month_avg := v_cpu_host_month_avg + nvl(round(metric_row.h_cpu_per_s,2),0);
				v_IOPS_month_avg := v_IOPS_month_avg + nvl(round((metric_row.read_iops+metric_row.write_iops),2),0);		
				...
				if(metric_row.h_cpu_per_s_max>=v_cpu_host_month_max) then 
					v_cpu_host_month_max := round(metric_row.h_cpu_per_s_max,2); 
				end if;
				if((metric_row.read_iops_max+metric_row.write_iops_max)>=v_IOPS_month_max) then 
					v_IOPS_month_max := round((metric_row.read_iops_max+metric_row.write_iops_max),2);
				end if;
			...
			if current_week < &&WEEKS_NUMBER then								
				...
				v_cpu_host_weekly_avg(current_week+1) := v_cpu_host_weekly_avg(current_week+1) + nvl(round(metric_row.h_cpu_per_s,2),0);
				v_IOPS_weekly_avg(current_week+1) := v_IOPS_weekly_avg(current_week+1) + nvl(round(metric_row.read_iops+metric_row.write_iops,2),0);
				...
				if(metric_row.h_cpu_per_s_max>=v_cpu_host_weekly_max(current_week+1)) then
					v_cpu_host_weekly_max(current_week+1) := round(metric_row.h_cpu_per_s_max,2); 
				end if;
				if(metric_row.read_iops_max+metric_row.write_iops_max>=v_IOPS_weekly_max(current_week+1)) then
					v_IOPS_weekly_max(current_week+1) := round(metric_row.read_iops_max+metric_row.write_iops_max,2);
				end if;
			...
			if current_day_difference < &&WEEK then		
				if current_day_difference<7 then 
					...
					v_cpu_host_daily_avg(current_day_difference+1) := v_cpu_host_daily_avg(current_day_difference+1) + nvl(round(metric_row.h_cpu_per_s,2),0);
					v_IOPS_daily_avg(current_day_difference+1) := v_IOPS_daily_avg(current_day_difference+1) + nvl(round(metric_row.read_iops+metric_row.write_iops,2),0);
					...
					if(metric_row.h_cpu_per_s_max>=v_cpu_host_daily_max(current_day_difference+1)) then
						v_cpu_host_daily_max(current_day_difference+1) := round(metric_row.h_cpu_per_s_max,2); 
					end if;
					if(metric_row.read_iops_max+metric_row.write_iops_max>=v_IOPS_daily_max(current_day_difference+1)) then
						v_IOPS_daily_max(current_day_difference+1) := round(metric_row.read_iops_max+metric_row.write_iops_max,2);
					end if;
			...
			if sysdate - metric_row.end_time < 1 then
				...
				v_time_series_element := concat(v_time_series_element, concat(nvl(to_char(round(metric_row.h_cpu_per_s,2)),'N/A'), '|||'));
				v_time_series_element := concat(v_time_series_element, concat(nvl(to_char(round(metric_row.h_cpu_per_s_max,2)),'N/A'), '|||'));		
				v_time_series_element := concat(v_time_series_element, concat(nvl(to_char(round(metric_row.read_iops+metric_row.write_iops,2)),'N/A'), '|||'));
				v_time_series_element := concat(v_time_series_element, concat(nvl(to_char(round(metric_row.read_iops_max+metric_row.write_iops_max,2)),'N/A'), '|||'));
		...
		if(day_month_count>=&&MONTH_SOFT_LIMIT) then 
			...
			if(snap_count_month>0) then
				...
				v_cpu_host_month_avg := round((v_cpu_host_month_avg / snap_count_month),2);
				v_IOPS_month_avg := round((v_IOPS_month_avg / snap_count_month),2);
				v_IOMB_month_avg := round((v_IOMB_month_avg / snap_count_month),2);
			end if;
			--v_cpu_db_month_avg,v_cpu_db_month_max,v_cpu_host_month_avg,v_cpu_host_month_max,v_IOPS_month_avg,v_IOPS_month_max,v_IOMB_month_avg,v_IOMB_month_max
			DBMS_OUTPUT.PUT_LINE(v_cpu_db_month_avg || '|||' || v_cpu_db_month_max || '|||' || v_cpu_host_month_avg || '|||' || v_cpu_host_month_max || '|||' || v_IOPS_month_avg || '|||' || v_IOPS_month_max || '|||' || v_IOMB_month_avg || '|||' || v_IOMB_month_max);
		else
			DBMS_OUTPUT.PUT_LINE('N/A|||N/A|||N/A|||N/A|||N/A|||N/A|||N/A|||N/A');
		end if;
		...
		LOOP
			...
			if (day_week_count(w_index)>=&&WEEK_SOFT_LIMIT) THEN
				if  (snap_count_week(w_index)>0) THEN
					...
					v_cpu_host_weekly_avg(w_index) := round((v_cpu_host_weekly_avg(w_index) / snap_count_week(w_index)),2);
					...
					--v_cpu_db_weekly_avg(w_index),v_cpu_db_weekly_max(w_index),v_cpu_host_weekly_avg(w_index),v_cpu_host_weekly_max(w_index),v_IOPS_weekly_avg(w_index),v_IOPS_weekly_max(w_index),v_IOMB_weekly_avg(w_index),v_IOMB_weekly_max(w_index)
					DBMS_OUTPUT.PUT_LINE(v_cpu_db_weekly_avg(w_index) || '|||' || v_cpu_db_weekly_max(w_index) || '|||' || v_cpu_host_weekly_avg(w_index) || '|||' || v_cpu_host_weekly_max(w_index) || '|||' || v_IOPS_weekly_avg(w_index) || '|||' || v_IOPS_weekly_max(w_index) || '|||' || v_IOMB_weekly_avg(w_index) || '|||' || v_IOMB_weekly_max(w_index));
				ELSE
					DBMS_OUTPUT.PUT_LINE('N/A|||N/A|||N/A|||N/A|||N/A|||N/A|||N/A|||N/A');
				end if;
			ELSE
		...
		--Output daily averages and maximums last week
		--Loops over days
		for d_index in daily_count.FIRST..daily_count.LAST
		loop 
		--the array starts from position 1 (today->position 1 in the array, 7 days ago->position 7 in the array)
			if(daily_count(d_index)>0) then
				v_cpu_db_daily_avg(d_index) := round(v_cpu_db_daily_avg(d_index) / daily_count(d_index),2);
				v_cpu_host_daily_avg(d_index) := round(v_cpu_host_daily_avg(d_index) / daily_count(d_index),2);
				v_IOPS_daily_avg(d_index) := round(v_IOPS_daily_avg(d_index) / daily_count(d_index),2);
				v_IOMB_daily_avg(d_index) := round(v_IOMB_daily_avg(d_index) / daily_count(d_index),2);
				--v_cpu_db_daily_avg(d_index),v_cpu_db_daily_max(d_index),v_cpu_host_daily_avg(d_index),v_cpu_host_daily_max(d_index),v_IOPS_daily_avg(d_index),v_IOPS_daily_max(d_index),v_IOMB_daily_avg(d_index),v_IOMB_daily_max(d_index)
				DBMS_OUTPUT.PUT_LINE(v_cpu_db_daily_avg(d_index) || '|||' || v_cpu_db_daily_max(d_index) || '|||' || v_cpu_host_daily_avg(d_index) || '|||' || v_cpu_host_daily_max(d_index) || '|||' || v_IOPS_daily_avg(d_index) || '|||' || v_IOPS_daily_max(d_index) || '|||' || v_IOMB_daily_avg(d_index) || '|||' || v_IOMB_daily_max(d_index));
			else
				DBMS_OUTPUT.PUT_LINE('N/A|||N/A|||N/A|||N/A|||N/A|||N/A|||N/A|||N/A');
			end if;		
		END LOOP;
