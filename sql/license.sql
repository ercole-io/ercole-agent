...
--20260209 Dal 2018 'Privilege Capture' non è più incluso nel vault per cui non è più a pagamento (non va considerato) - https://www.oracle.com/a/tech/docs/dbsec/pa/faq-pa-19c.pdf
         ...
         when (select version from v$instance) like '11%' and (select usage from TAB where LTRIM(product,'.') like 'Real Application Clusters') is NOT NULL and '&3'='xOne' and (select count(*) from gv$instance) = 1 
		  ...
		  WHEN LTRIM(product,'.') LIKE 'Active Data Guard'
           ...
           when LTRIM(product,'.') LIKE 'Database Gateway'
            AND (select count(replace(replace(replace(to_char(substr(FEATURE_INFO, 1, 1000)), chr(10), '[LF]'), chr(13), '[CR]'),'"','''')) from dba_feature_usage_Statistics
