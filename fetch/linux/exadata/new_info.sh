...
function checkRoot {
    export CURRENT_USR=$(whoami)
        if [[ "$CURRENT_USR" != "root" ]]; then
            echo "--> ERROR: utility cannot be executed as non-root user (current user: ${CURRENT_USR}). Please switch to root and try again"
            echo "Exiting..."
            exit 1
        fi
}
...
function getRackID {
    export RACK_ID=$(ipmitool sunoem cli 'show /SP system_identifier'|grep '='|awk '{ print $7 }')
}
...
function HostGetDetails {
    echo "HOST_TYPE|||RACK_ID|||HOSTNAME|||CPU_ENABLED|||CPU_TOT|||MEMORY_GB|||IMAGEVERSION|||KERNEL|||MODEL|||FAN_USED|||FAN_TOTAL|||PSU_USED|||PSU_TOTAL|||MS_STATUS|||RS_STATUS"
    while read NHOST
    do
        INFO=$(dcli -c $NHOST -l root "dbmcli -e list dbserver attributes name,cpuCount,fanCount,kernelVersion,powerCount,releaseVersion,msStatus,rsStatus"| sed "s/${NHOST}: //g")
        ...
        MODEL_APP=$(dcli -c $NHOST -l root "ipmitool sunoem cli 'show /SYSTEM system_identifier'|grep system_identifier|grep -iv show"|sed "s/${NHOST}: //g"|awk '{print $3 " " $4 " " $5 " " $6}')
        MODEL_TMP=$(dcli -c $NHOST -l root "ipmitool sunoem cli 'show /SYSTEM component_model'|grep component_model|grep -iv show"|sed "s/${NHOST}: //g"|awk '{print $5}')
...
function CellGetDetails {
    ...
    echo "HOST_TYPE|||RACK_ID|||HOST|||CPU_ENABLED|||CPU_TOT|||MEMORY_GB|||IMAGEVERSION|||KERNEL|||MODEL|||FAN_USED|||FAN_TOTAL|||PSU_USED|||PSU_TOTAL|||CELLSRV_STATUS|||MS_STATUS|||RS_STATUS"
    while read CELL 
    do
        ...
        INFO_APP=$(dcli -c $CELL -l root "cellcli -e list cell attributes rsStatus"| sed "s/${CELL}: //g")
...
function vmGetDetails {
    ...
    while read NHOST
    do
        INFO=$(dcli -c $NHOST -l root "dbmcli -e list dbserver attributes name,kernelVersion,releaseVersion,msStatus,rsStatus"| sed "s/${NHOST}: //g")
        HOST=$(echo $INFO|awk '{print $1}')
...
function KVMGetVMResources {
    ...
    while read NHOST
    do
        VM_APP=$(dcli -c $NHOST -l root vm_maker --list --domain|grep $RUN_FLAG running|awk -F'[(]' '{print $1}')
        VM_LIST=$(echo $VM_APP| sed "s/${NHOST}: //g")
        dcli -g $DBS_LST -l root "vm_maker --list --vcpu|grep 'Current:' > $VM_CPU_LST"
        dcli -g $DBS_LST -l root "vm_maker --list --memory > $VM_RAM_LST"

        for VM in $VM_LIST
        do
            CPU_CURRENT_APP=$(dcli -c $NHOST -l root grep $VM $VM_CPU_LST|awk '{print $4}')
            CPU_CURRENT=$(echo $CPU_CURRENT_APP|sed "s/${NHOST}: //g")
            CPU_RESTART_APP=$(dcli -c $NHOST -l root grep $VM $VM_CPU_LST|awk '{print $6}')
            CPU_RESTART=$(echo $CPU_RESTART_APP|sed "s/${NHOST}: //g")
            RAM_CURRENT_APP=$(dcli -c $NHOST -l root grep $VM $VM_RAM_LST|awk '{print $3}')
            RAM_CURRENT=$(echo $RAM_CURRENT_APP|sed "s/${NHOST}: //g")
            RAM_RESTART_APP=$(dcli -c $NHOST -l root grep $VM $VM_RAM_LST|awk '{print $4}')
    ...
    dcli -g $DBS_LST -l root "rm -f $VM_CPU_LST $VM_RAM_LST"
}
...
function OVMGetVMResources {
    ...
    while read NHOST
    do
        VM_LIST=$(dcli -c $NHOST -l root xm list|grep -v Domain-0|grep -v VCPUs|awk '{print $2}')
        for VM in $VM_LIST
        do
            dcli -c $NHOST -l root "xm list $VM -l > $VM_DETAILS"
            CPU_ONLINE=$(dcli -c $NHOST -l root grep -i online_vcpus $VM_DETAILS|awk '{print $3}'|sed "s/)//g")
            CPU_MAX_USABLE=$(dcli -c $NHOST -l root grep -i vcpus $VM_DETAILS|grep -iv online|awk '{print $3}'|sed "s/)//g")
            RAM_ONLINE=$(dcli -c $NHOST -l root grep -i memory $VM_DETAILS|grep -iv memory_|grep -iv shadow|awk '{print $3}'|sed "s/)//g")
            RAM_MAX_USABLE=$(dcli -c $NHOST -l root grep -i memory_actual $VM_DETAILS|awk '{print $3}'|sed "s/)//g")
            echo "$HOST_TYPE|||$NHOST|||$VM|||$CPU_ONLINE|||$CPU_MAX_USABLE|||$RAM_ONLINE|||$RAM_MAX_USABLE"
        done
    done < $DBS_LST
    dcli -g $DBS_LST -l root "rm -f $VM_DETAILS"
}
...
case $1 in 
    ...
    ;;
    *)
        echo " --> ERROR: the parameter given as input to the shell is wrong ($1). The supported ones are -cell/-ibs or blank"
        echo "            Please review the syntax"
        echo " --> Exiting..."
        exit 1;
    ;;
esac