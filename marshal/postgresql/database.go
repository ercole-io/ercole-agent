...
func Database(cmdOutput []byte) (*model.PostgreSQLDatabase, error) {
	scanner := bufio.NewScanner(bytes.NewReader(cmdOutput))
	result := model.PostgreSQLDatabase{}
	...
	for scanner.Scan() {
		...
		if result.SchemasCount, err = strconv.Atoi(iter()); err != nil {
			merr = multierror.Append(merr, err)
		}

		if result.DbSize, err = strconv.Atoi(iter()); err != nil {
			merr = multierror.Append(merr, err)
		}
		...
		if result.LobsCount, err = strconv.Atoi(iter()); err != nil {
			merr = multierror.Append(merr, err)
		}
		...
		if len(splitted) > 12 {
			...
			if result.ExtensionsCount, err = strconv.Atoi(iter()); err != nil {
				merr = multierror.Append(merr, err)
			}

			result.LogicReplSetup = marshal.TrimParseBool(iter())

			if result.PublicationsCount, err = strconv.Atoi(iter()); err != nil {
				merr = multierror.Append(merr, err)
			}

			if result.SubscriptionsCount, err = strconv.Atoi(iter()); err != nil {
				merr = multierror.Append(merr, err)
			}
		}
	}
